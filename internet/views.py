...
from .models import Package, ContactMessage, BusinessQuoteRequest, UserProfile, SpeedTestResult, Branch
...
def speed_test(request):
    if request.method == 'POST':
        try:
            ...
            st.get_best_server()
            logger.debug("Performing download test...")
            download_speed = st.download() / 1_000_000  # Convert to Mbps
            logger.debug("Performing upload test...")
            upload_speed = st.upload() / 1_000_000 # Convert to Mbps
            ping = st.results.ping
            ip_address = request.META.get('REMOTE_ADDR', 'Unknown')
            user = request.user if request.user.is_authenticated else None
            
            logger.debug(f"Saving speed test result: download={download_speed}, upload={upload_speed}, ping={ping}")
            ...
            return JsonResponse({'error': f"Speed test failed: {str(e)}"}, status=500)
